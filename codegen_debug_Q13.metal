...
// === Phase 0: Q13_count_orders ===
kernel void Q13_count_orders(
    device atomic_uint* d_order_counts [[buffer(0)]],
    device const int* o_custkey [[buffer(1)]],
    device const char* o_comment [[buffer(2)]],
    ...
    for (uint i = tid; i < n_orders; i += tpg) {
        if (!q13_comment_match(o_comment, i)) {
            atomic_fetch_add_explicit(&d_order_counts[o_custkey[i]], 1u, memory_order_relaxed);
        }
    }
}

// === Phase 1: Q13_build_histogram ===
kernel void Q13_build_histogram(
    device atomic_uint* d_histogram [[buffer(0)]],
    device int* d_order_counts [[buffer(1)]],
    ...
    for (uint i = tid; i < n_customer; i += tpg) {
        int _cnt = d_order_counts[c_custkey[i]];
        if (_cnt != 2147483647) {
            atomic_fetch_add_explicit(&d_histogram[_cnt], 1u, memory_order_relaxed);
        }
    }
}
