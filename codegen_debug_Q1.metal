...
kernel void Q1_reduce(
    device atomic_uint* d_q1_aggs [[buffer(0)]],
    device const float* l_discount [[buffer(1)]],
    device const float* l_extendedprice [[buffer(2)]],
    device const char* l_linestatus [[buffer(3)]],
    device const float* l_quantity [[buffer(4)]],
    device const char* l_returnflag [[buffer(5)]],
    device const int* l_shipdate [[buffer(6)]],
    device const float* l_tax [[buffer(7)]],
    constant uint& n_lineitem [[buffer(8)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        if (l_shipdate[i] <= 19980902) {
            int _bucket = ((l_returnflag[i] == 'A' ? 0 : (l_returnflag[i] == 'N' ? 2 : 4)) + (l_linestatus[i] == 'F' ? 0 : 1));
            atomic_add_long_pair(&d_q1_aggs[_bucket * 10 + 0], &d_q1_aggs[_bucket * 10 + 1], (long)((uint)(l_quantity[i] * 100.0f)));
            atomic_add_long_pair(&d_q1_aggs[_bucket * 10 + 2], &d_q1_aggs[_bucket * 10 + 3], (long)((uint)(l_extendedprice[i] * 100.0f)));
            atomic_add_long_pair(&d_q1_aggs[_bucket * 10 + 4], &d_q1_aggs[_bucket * 10 + 5], (long)((uint)(l_extendedprice[i] * (1.0f - l_discount[i]) * 100.0f)));
            atomic_add_long_pair(&d_q1_aggs[_bucket * 10 + 6], &d_q1_aggs[_bucket * 10 + 7], (long)((uint)(l_extendedprice[i] * (1.0f - l_discount[i]) * (1.0f + l_tax[i]) * 100.0f)));
            atomic_fetch_add_explicit(&d_q1_aggs[_bucket * 10 + 8], (uint)((uint)(l_discount[i] * 10000.0f)), memory_order_relaxed);
            atomic_fetch_add_explicit(&d_q1_aggs[_bucket * 10 + 9], (uint)(1u), memory_order_relaxed);
        }
    }
}
