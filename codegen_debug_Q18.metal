...
// === Phase 0: Q18_aggregate ===
kernel void Q18_aggregate(
    device atomic_float* d_order_qty [[buffer(0)]],
    device const int* l_orderkey [[buffer(1)]],
    device const float* l_quantity [[buffer(2)]],
    constant uint& n_lineitem [[buffer(3)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        atomic_fetch_add_explicit(&d_order_qty[l_orderkey[i]], (float)(l_quantity[i]), memory_order_relaxed);
    }
}
