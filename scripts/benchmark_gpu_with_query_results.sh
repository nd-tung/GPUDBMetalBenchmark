#!/bin/bash
# GPU Metal TPC-H Benchmark Script — with full query output + log files
# Usage: benchmark_gpu_with_query_results.sh [sf1|sf10|sf100|all]
...
# ---------------------------------------------------------------------------
# Parse output → CSV + per-query log files
# ---------------------------------------------------------------------------
parse_and_record() {
  local sf_label="$1"
  local out_file="$2"
...
# ---------------------------------------------------------------------------
# Run benchmark (with tee for live output)
# ---------------------------------------------------------------------------
run_benchmark() {
  local sf_arg="$1"
  local sf_label="$2"
...
echo "Results CSV: ${GPU_CSV}"
echo "Log directory: ${LOG_DIR}/${TIMESTAMP}/"
echo ""
