...
kernel void Q6_reduce(
    device atomic_uint* d_revenue_revenue_lo [[buffer(0)]],
    device atomic_uint* d_revenue_revenue_hi [[buffer(1)]],
    device const float* l_discount [[buffer(2)]],
    device const float* l_extendedprice [[buffer(3)]],
    device const float* l_quantity [[buffer(4)]],
    device const int* l_shipdate [[buffer(5)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        if ((l_shipdate[i] >= 19940101) && (l_shipdate[i] < 19950101) && ((l_discount[i] >= (0.060000f - 0.010000f) && l_discount[i] <= (0.060000f + 0.010000f))) && (l_quantity[i] < 24)) {
            local_revenue += (long)((long)((l_extendedprice[i] * l_discount[i]) * 100.0f));
        }
    }
    ...
    if (lid == 0) {
        atomic_add_long_pair(d_revenue_revenue_lo, d_revenue_revenue_hi, tg_revenue);
    }
}
