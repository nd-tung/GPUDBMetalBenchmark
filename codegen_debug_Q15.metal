...
// === Phase 0: Q15_aggregate ===
kernel void Q15_aggregate(
    device atomic_float* d_supp_revenue [[buffer(0)]],
    device const int* l_suppkey [[buffer(1)]],
    device const int* l_shipdate [[buffer(2)]],
    device const float* l_extendedprice [[buffer(3)]],
    device const float* l_discount [[buffer(4)]],
    constant uint& n_lineitem [[buffer(5)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        if (l_shipdate[i] >= 19960101 && l_shipdate[i] < 19960401) {
            atomic_fetch_add_explicit(&d_supp_revenue[l_suppkey[i]], (float)(l_extendedprice[i] * (1.0f - l_discount[i])), memory_order_relaxed);
        }
    }
}
