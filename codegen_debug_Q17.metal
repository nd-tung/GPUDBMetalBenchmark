...
// === Phase 0: Q17_reduce ===
kernel void Q17_reduce(
    device atomic_uint* d_q17_revenue_lo [[buffer(0)]],
    device atomic_uint* d_q17_revenue_hi [[buffer(1)]],
    device float* d_q17_threshold [[buffer(2)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        float _thresh = d_q17_threshold[l_partkey[i]];
        if (_thresh != 0) {
            if (l_quantity[i] < _thresh) {
                local_revenue += (long)((long)(l_extendedprice[i] * 100.0f));
            }
        }
    }
    ...
    if (lid == 0) {
        atomic_add_long_pair(d_q17_revenue_lo, d_q17_revenue_hi, tg_revenue);
    }
}
