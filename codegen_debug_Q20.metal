...
// === Phase 0: Q20_lineitem_agg ===
kernel void Q20_lineitem_agg(
    constant uint& d_q20_ht_mask [[buffer(0)]],
    constant uint& supp_mul [[buffer(1)]],
    device const uint* d_q20_ht_keys [[buffer(2)]],
    device atomic_float* d_q20_ht_vals [[buffer(3)]],
    device const uint* d_q20_part_bitmap [[buffer(4)]],
    device const int* l_partkey [[buffer(5)]],
    device const int* l_suppkey [[buffer(6)]],
    device const float* l_quantity [[buffer(7)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        if (l_shipdate[i] >= 19940101 && l_shipdate[i] < 19950101) {
            if (bitmap_test(d_q20_part_bitmap, l_partkey[i])) {
                int _unused = (q20_ht_add(d_q20_ht_keys, d_q20_ht_vals, d_q20_ht_mask, (uint)l_partkey[i] * supp_mul + (uint)l_suppkey[i], l_quantity[i]), 0);
            }
        }
    }
}
