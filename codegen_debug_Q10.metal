...
// === Phase 0: Q10_build_orders_map ===
kernel void Q10_build_orders_map(
    ...
    constant uint& n_orders [[buffer(4)]],
    ...
    for (uint i = tid; i < n_orders; i += tpg) {
        if ((o_orderdate[i] >= 19931001) && (o_orderdate[i] < 19940101)) {
            d_orders_map[o_orderkey[i]] = o_custkey[i];
        }
    }
}

// === Phase 1: Q10_probe_aggregate ===
kernel void Q10_probe_aggregate(
    device atomic_float* d_cust_revenue [[buffer(0)]],
    device int* d_orders_map [[buffer(1)]],
    device const int* l_orderkey [[buffer(2)]],
    device const char* l_returnflag [[buffer(3)]],
    ...
    for (uint i = tid; i < n_lineitem; i += tpg) {
        if (l_returnflag[i] == 'R') {
            int _custkey = d_orders_map[l_orderkey[i]];
            if (_custkey != -1) {
                atomic_fetch_add_explicit(&d_cust_revenue[_custkey], (float)(l_extendedprice[i] * (1.0f - l_discount[i])), memory_order_relaxed);
            }
        }
    }
}
